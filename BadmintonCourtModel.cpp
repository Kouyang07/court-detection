...
void BadmintonCourtModel::writeToFile(const std::string& filename)
{
  ...
  // Write all 32 intersections
  for (int i = 0; i < transformedModelPoints.size(); i++) {
    auto point = transformedModelPoints[i];
    outFile << "P" << i + 1 << "," << point.x << "," << point.y << std::endl;
  }

  // Write the two net poles
  for (int i = 0; i < 2; i++) {
    auto point = netPoints[i];
    outFile << "NetPole" << i + 1 << "," << point.x << "," << point.y << std::endl;
  }
}